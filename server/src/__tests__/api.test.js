...
describe("Pétition des résidents", () => {
  const sign = (body) => request(app).post("/api/petitions/oiseaux/sign").send(body);
  const valid = { nom: "Dupont", prenom: "Élodie", logement: "B12", email: "", consentement: true };

  it("enregistre une signature sans authentification", async () => {
    expect((await sign(valid)).status).toBe(201);
  });

  it("refuse un doublon (accents, casse et espaces ignorés)", async () => {
    const res = await sign({ ...valid, nom: "  DUPONT ", prenom: "elodie", logement: "b12" });
    expect(res.status).toBe(409);
  });

  it("refuse sans consentement ou avec un e-mail invalide", async () => {
    expect((await sign({ ...valid, nom: "X", consentement: false })).status).toBe(400);
    expect((await sign({ ...valid, nom: "Y", email: "pas-un-email" })).status).toBe(400);
  });

  it("refuse une pétition inconnue", async () => {
    const res = await request(app).post("/api/petitions/inconnue/sign").send({ ...valid, nom: "Z" });
    expect(res.status).toBe(404);
  });

  it("ne laisse lire les signatures ni à un gestionnaire, ni sans jeton, ni avec un mauvais jeton", async () => {
    process.env.PETITION_ADMIN_TOKEN = "jeton-de-test-suffisamment-long";
    const asManager = await request(app).get("/api/petitions/oiseaux/signatures").set("Authorization", `Bearer ${managerToken}`);
    expect(asManager.status).toBe(403);
    expect((await request(app).get("/api/petitions/oiseaux/signatures")).status).toBe(403);
    expect((await request(app).get("/api/petitions/oiseaux/signatures").set("x-admin-token", "mauvais")).status).toBe(403);
  });

  it("donne la liste avec le bon jeton, et reste fermé si aucun jeton n'est configuré", async () => {
    const ok = await request(app).get("/api/petitions/oiseaux/signatures").set("x-admin-token", "jeton-de-test-suffisamment-long");
    expect(ok.status).toBe(200);
    expect(ok.body.count).toBeGreaterThanOrEqual(1);
    delete process.env.PETITION_ADMIN_TOKEN;
    const closed = await request(app).get("/api/petitions/oiseaux/signatures").set("x-admin-token", "");
